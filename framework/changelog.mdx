...
- Add `AHeaderSubtitle` component
- Fix hidden cursor on documentation website's search input
- Add `expandable` prop to `ADataTable` to configure expandable table rows
- Expose state setter from `useADateRange` hook to control `ADatePicker` state
...
- Update styles to match atomic 3.3.0, including:
  - `AButton`
  - `ADatePicker`
  - `APagination`
  - `ASlider`
- Support automatic system theme detection with `AAutoTheme` and `useAAutoTheme`

## 3.2.1 (11-17-2021)

- Added icons: `one-click-bidirectional` & `one-click-unidirectional`

## 3.2.0 (11-17-2021)

- Added `AStepper`, `AStep`, `AStepTitle`, `AStepDescription` components
- Removed `tabindex` from `AAlert`, `AHeaderTitle`, as those are not interactive elements
- Removed `role` from `ACheckbox` and `ARadio`, as they were redundant
...
- Set `AAutocomplete`, `ACombobox`, `ASelect` menu min-width to be `max-content` and for `ASelect` add right padding to account for checkmark

## 3.1.1 (2021-09-16)

- Unregister fields from `AForm` on unmount
