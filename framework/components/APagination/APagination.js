          ...
          <>
            <div className="a-pagination__results-per-page">
              <ATextInput
                ...
                onChange={(e) => {
                  // x > 1
                  const greaterThan1 = Math.max(parseInt(e.target.value), 1);

                  // x < maxResultsPerPage if maxResultsPerPage > 0
                  const smallerThanMax = Math.min(
                    greaterThan1,
                    parseInt(maxResultsPerPage) || greaterThan1
                  );

                  setWorkingResultsPerPage(Math.min(smallerThanMax, total));
                }}
                onKeyDown={(e) => {
...
APagination.propTypes = {
  ...
  /**
   * Sets the maximum results per page. It's optional and when not set there's no max.
   * Note you can bypass this by setting/changing the `resultsPerPage` prop.
   */
  maxResultsPerPage: PropTypes.number,
  /**
