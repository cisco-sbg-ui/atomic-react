...
<Meta title="Components/Autocompletes" component={AAutocomplete} />
      ...
      <YourComponent>
        {() => {
          ...
          */
          ...
          const searchCallback = async (rawTerm) => {
            ...
            setItems(
              hits.map((x) => ({
                ...
                value: `${x.title}_${x.author}_${x.year}`,
                title: x.title,
                author: x.author,
                year: x.year
              }))
...
the `AAutocomplete` will select and focus previous/next items on `up` or `down` keys for the `keydown` event.
