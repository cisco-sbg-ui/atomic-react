...
function build-and-publish-package {
  ...
  if [ "${PKG_TYPE}" == "int" ]; then
    ...
    docker run -v /home/travis/build/cisco-sbg-ui/atomic-react:/code/ -e CYBRIC_API_KEY="${CYBRIC_API_KEY}" -e POLICY_ID=finNBo6IR1i-vB196ADgwA -e WORKSPACE=/home/travis/build/cisco-sbg-ui/atomic-react -v /var/run/docker.sock:/var/run/docker.sock --name zeronorth zeronorth/integration:latest python cybric.py
